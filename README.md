...
Rows are never dropped. Every step that comes back empty leaves a `*_miss_reason` saying why (`no_jobs`, `no_email`, `domain_mismatch:bit.ly`, `status_invalid`, `http_403`, ...). An email whose verification status is not `valid` or `valid_catch_all` is kept for the record but carries `email_miss_reason=status_<x>`, so filter on that before outreach. `email_catch_all` is true when the domain accepts any address, so a `valid_catch_all` hit is found but not proven deliverable. Company records the provider returns for a different domain are quarantined: only the miss reason is kept. `funding_miss_reason=possibly_partial:20_rounds` means the total is real but the provider page was full, so later rounds may be missing. For people, `last_enriched_at` is the newest fetch among the lookups the current run used (people search, email, optional mobile); it can move backwards on a cached or partial rerun and does not date every stored value. Use `email_verified_at` for email freshness.
